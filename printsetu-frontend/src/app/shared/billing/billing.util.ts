...
export const STATE_META: Record<SubscriptionStateOrNone, { label: string; tone: Tone; hint: string }> = {
  ...
  PAYMENT_PENDING: { label: 'Payment pending', tone: 'warn', hint: 'Renewal date passed. Full access for a few more days.' },
  PAST_DUE: { label: 'Payment overdue', tone: 'bad', hint: 'Extra days to pay are over. Old orders only, no new print requests.' },
...
export const EVENT_META: Record<string, { label: string; icon: string; tone: Tone }> = {
  ...
  GRACE_EXTENDED: { label: 'Extra days to pay given', icon: 'pi-hourglass', tone: 'warn' },
  ...
  PAST_DUE: { label: 'Payment became overdue', icon: 'pi-exclamation-circle', tone: 'bad' },
  ...
  FORCE_SUSPENDED: { label: 'Shop paused by admin', icon: 'pi-lock', tone: 'bad' },
  FORCE_REACTIVATED: { label: 'Shop turned back on by admin', icon: 'pi-lock-open', tone: 'ok' },
  OVERRIDE_RELEASED: { label: 'Back to automatic rules', icon: 'pi-unlock', tone: 'info' },
  OVERRIDE_EXPIRED: { label: 'Manual control ended', icon: 'pi-unlock', tone: 'info' },
